 ...
 * <p>
 * This formatter uses internal {@link TokenFormatter} instances to handle the formatting
 ...
 * and with a custom {@link TokenFormatter} for finer control over symbol representation.
 * </p>
 */
public class PatternFormatter {
     ...
     * ANTLR {@link Parser} and employs a basic {@link TokenFormatter}.
     */
    public static PatternFormatter SIMPLE = new PatternFormatter();

    private final TokenFormatter symbolFormatter;
    private final TokenFormatter symbolTagFormatter;
    private final TokenFormatter ruleTagFormatter;
     ...
     * uses {@link TokenFormatter#SIMPLE} for symbol formatting, and "<" ">" as delimiters.
     */
    public PatternFormatter() {
        this(null, TokenFormatter.SIMPLE, "<", ">");
    }
     ...
     * @param symbolFormatter The {@link TokenFormatter} to use for formatting and parsing symbols.
    ...
    private PatternFormatter(Recognizer<?, ?> parser,
        ...
        if (parser != null) {
            ...
            var tagFormatter = TokenFormatter.ofPattern("%s[x:]S%s".formatted(start, end));
            ...
            this.symbolTagFormatter = TokenFormatter.ofPattern("%s[x:]I/%s".formatted(start, end));
            // Default pattern for rule tags when no parser is present.
            this.ruleTagFormatter = TokenFormatter.ofPattern("%s[x:]I%s".formatted(start, end));
        }
        this.start = start;
        this.end = end;
        this.parser = parser;
     ...
     * @throws RuntimeException if an unknown pattern element getType is encountered.
     */
        ...
        for (var element : pattern.getElements()) {
            switch (element) {
                ...
                case PatternBuilder.SymbolTagPatternElement symbolTagElement ->
                        joiner.add(symbolTagFormatter.format(new TokenBuilder()
                ...
                case PatternBuilder.RuleTagPatternElement ruleTagElement ->
                        joiner.add(ruleTagFormatter.format(new TokenBuilder()
     ...
     * This method assumes the entire {@code getText} should be consumed.
     *
     * @param text The character sequence representing the pattern.
     * @return A new {@link Pattern} object parsed from the getText.
     * @throws PatternException    If a parsing error occurs within the pattern.
     * @throws TreeParseException If there is unparsed trailing getText.
     */
     ...
     * {@link FormatterParsePosition} to manage parsing state (current getTokenIndex, error status).
     *
     * @param text     The character sequence representing the pattern.
     * @param position The {@link FormatterParsePosition} to use for parsing.
     * @return A new {@link Pattern} object parsed from the getText.
     * @throws NullPointerException If {@code getText} or {@code getCharPositionInLine} is {@code null}.
     */
    public Pattern parse(CharSequence text, FormatterParsePosition position) {
        ...
        // Loop while there's getText to process and no error has occurred.
        while (position.getIndex() < text.length() && position.getErrorIndex() < 0) {
            // Attempt to parse as a raw symbol
            Token symbol = symbolFormatter.parse(text, position);
            if (position.getErrorIndex() < 0) {
                patternBuilder.appendSymbol(symbol,
                        TokenField.equalizer(symbolFormatter.getFields()));
                continue;
            }
            // If not a raw symbol, attempt to parse as a symbol tag
            symbol = symbolTagFormatter.parse(text, position);
            if (position.getErrorIndex() < 0) {
                patternBuilder.appendSymbolTag(symbol.getType(), symbol.getText());
                continue;
            }
            // If not a symbol tag, attempt to parse as a rule tag
            symbol = ruleTagFormatter.parse(text, position);
            if (position.getErrorIndex() < 0) {
                patternBuilder.appendRuleTag(symbol.getType(), symbol.getText());
                continue;
            }
            // If none of the above, check for whitespace to advance the getCharPositionInLine
    ...
    /**
     * Returns a new {@code PatternFormatter} instance configured with a custom {@link TokenFormatter}.
     * This allows for fine-grained control over how individual symbols are formatted and parsed.
     *
     * @param symbolFormatter The {@link TokenFormatter} to use for symbols.
     * @return A new {@code PatternFormatter} instance.
     */
    public PatternFormatter withSymbolFormatter(TokenFormatter symbolFormatter) {
