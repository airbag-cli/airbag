...
import io.github.airbag.gen.SchemaBaseVisitor;
import io.github.airbag.gen.SchemaLexer;
import io.github.airbag.gen.SchemaParser;
import io.github.airbag.gen.SchemaVisitor;
...
public class TokenProvider {
     ...
     * {@code Schema.g4} grammar. It is designed for creating precise token sequences,
     ...
     * The input string can contain a sequence of token specifications. According to the {@code Schema.g4}
    ...
    public List<Token> fromSpec(String input) {
        SchemaLexer schemaLexer = new SchemaLexer(CharStreams.fromString(input));
        SchemaParser schemaParser = new SchemaParser(new CommonTokenStream(schemaLexer));
        SchemaVisitor<List<Token>> visitor = new SchemaBaseVisitor<>() {

            @Override
            public List<Token> visitTokenList(SchemaParser.TokenListContext ctx) {
                return IntStream.range(0, ctx.token().size()).mapToObj(i -> visitToken(ctx.token(i), i)).toList();
            }

            private Token visitToken(SchemaParser.TokenContext tokenCtx, int i) {
        ...
        return visitor.visitTokenList(schemaParser.tokenList());
    }
